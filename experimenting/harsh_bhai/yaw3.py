...
K = np.array(
    [[675.537322, 0.000000, 311.191300], [0.000000, 677.852071, 221.610964], [0, 0, 1]]
)
...
for index in range(2498, 2593):  # Process images 2498-2592
    ...
    F, mask = cv2.findFundamentalMat(valid_prev, valid_curr, cv2.FM_RANSAC, 1.0, 0.99)
    ...
    # Recover pose
    _, R, t, _ = cv2.recoverPose(E, valid_prev, valid_curr, K, mask=mask)
    ...
    print(
        f"Frame {index}: ΔYaw = {np.degrees(yaw):.2f}°, Total = {np.degrees(total_yaw):.2f}°"
    )
